    ...
    <>
        <div className="flex flex-nowrap m-8 p-4 sm:flex-wrap  rounded-md border-slate-100 border-2 text-gray-700">
                {data && data.map((company)=>{
                    return(<>
                        <div onClick={handleCompany} className="hover:cursor-pointer bg-[#fff] hover:bg-slate-600 drop-shadow-md   border-b-8  border-b-slate-600 flex flex-col items-center rounded-lg w-72 p-2 m-2 h-[290px]">
                            <img src="" 
                            ...
                            <p className=" bg-white rounded-md border-2 font-semibold mt-3 p-1 w-64 ">PPT : {company.pre_placement_talk}</p>
                        </div>
                    </>)
                    })
                }
